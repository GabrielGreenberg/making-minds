...
1. `git -C <repo> branch --show-current`, `git status --porcelain`, `git log --oneline -1`.
   If the checkout is already on a `task/` branch, that task is the first resume candidate:
   read its file from `tasks/in-progress/` right there.
...
2. Claim on `main` (README §Lifecycle): `status: in-progress`, `branch: task/NNN-slug`,
   `git mv` → `in-progress/`, commit `tasks: claim NNN`. Then
...
land commit on the branch (`done/` + `log.md` line), merge `--no-ff` into `main`, delete
the branch. Push only if Gabriel says; then confirm `gh run list --limit 1` is green.
...
(`## Questions`, `status: blocked`, `git mv` → `blocked/`, commit on `main`), keep the branch,
note the branch name in the file.
