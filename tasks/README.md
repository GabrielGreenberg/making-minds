...
| `/work-loop` | `LOOP.md` | Attended loop, started as `/loop /work-loop`. `/work` without the picking: works the ready queue one task at a time (one Workflow per task), lands and pushes each, parks what needs you as questions, stops when nothing more can move. Releases by hand only when given `release`. |
