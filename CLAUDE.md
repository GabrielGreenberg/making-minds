...
_Last updated: 2026-09-11 (**accounts and sign-in — the launch auth system**. The
...
Earlier 2026-09-10: **the `notes/pset_updates.md` pass — 23 items, one commit each**.
...
- **Instructor side** — role-gated `#/instructor` mode: a **Roster & accounts** screen
  ...
  remove one person, approve or reject access requests), dashboard (with per-assignment ↑/↓
...
| Due dates     | `app/src/dueDates.ts`                                                          | Pure due-date display policy: `dueStatus` (green >3 days out / amber <3 days / red overdue — the home card's badge bands), `lateBy` (ms late; 0 = on time = no signal), `formatDuration`/`formatDueDate`. Consumed by HomeScreen + GradebookView; pinned by `tools/dueDateCheck.ts` |
