...
| Engine | `app/src/engine/cc.ts`, `sc.ts`, `netlist.ts`, `fsm.ts` | Pure simulators: topological eval (CC), clocked step (SC; boxes holding MEM inlined by `netlist.ts`, whose `memorySlots` is the state vector), transition matching (FSM; `evaluateFSMSymbolStep`). One `sortByLabel` orders I/O and box ports. |
