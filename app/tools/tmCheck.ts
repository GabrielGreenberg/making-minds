...
// Unary increment (two-action model): scan right, rewriting each 1 in place,
// and write a 1 into the first background cell.
...
//   S₀ on 1 → write 0, move L (stay S₀); S₀ on 0 → halt.
function tmZero() {
  return {
...
function tmAmbiguous() {
  return {
    components: [comp('s0', 'S₀')],
    wires: [wire('t1', 's0', 's0', '1:1R'), wire('t2', 's0', 's0', '1:1L')],
  };
}
...
check('parse: `*L`/`*R` actions are binary-only',
  parseTMAction('*R', 'binary') !== null && parseTMAction('*L', 'binary') !== null &&
  parseTMAction('*R', 'unary') === null && parseTMAction('*L', 'unary') === null);
check('parse: legacy single-char tokens are unparseable',
  parseTMAction('R', 'unary') === null && parseTMAction('0', 'unary') === null &&
  parseTMAction('1', 'binary') === null);
check('parse: bad direction char is unparseable',
  parseTMAction('0X', 'unary') === null && parseTMAction('1D', 'binary') === null);
check('parse: valid two-action tokens parse',
  parseTMAction('0L', 'unary') !== null && parseTMAction('1R', 'unary') !== null);
