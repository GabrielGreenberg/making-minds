...
function evalRow(
  ...
  const inputCells: PreviewCellInput[] = inputs.map((g, i) => {
    const v = values[i] ?? 0;
    vars[g.name] = v;
    return { name: g.name, value: v, bits: valueToBits(v, g.width, rep) };
  });
...
export function probeFormulas(
  ...
  const row = evalRow(inputs, outputs, rep, values, outputErrors);
...
export function buildExamples(
  ...
  const rows = combos.map((values) =>
    evalRow(inputs, outputs, rep, values, outputErrors),
  );
