...
// Two-action model: every step writes a symbol and moves. S₀ scans right over
// strokes, rewriting each `1` in place (`1:1R`, stay); on the first background
// cell it writes a stroke and moves right into the halting S₁ (`0:1R`) —
...
// Wrong: erase every stroke walking left (`1:0L`), then halt on the first
// background cell → always outputs 0. Collapses to a single state under the
// two-action model.
